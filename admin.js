...
function renderRequests() {
  ...
  document.getElementById("requestList").innerHTML = requests.length ? requests.map(item => `
    <article class="request-card" data-id="${Number(item.id)}">
      ...
      <div class="request-contact"><a href="tel:${escapeHtml(item.phone)}">اتصال</a><a href="https://wa.me/${normalizedPhone(item.phone)}?text=${encodeURIComponent(`مرحبًا ${item.name}، بخصوص طلبك ${item.ticket_code} لدى مدار التقنية.`)}" target="_blank" rel="noopener">واتساب</a></div>
